...
const NAV = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Prediccion IA',icon: 'ti-trending-up',     href: '/prediccion', desc: 'Analisis predictivo' },
  ...
  },
  {
    ...
    children: [
      { label: 'Auditorias', icon: 'ti-shield-check', href: '/auditoria', desc: 'MSPS, Supersalud' },
    ],
  },
  {
    label: 'Asistente IA',
      ...
      <aside style={{
        ...
        <nav style={{ flex:1, padding:'8px 0', overflowY:'auto' }}>
          {NAV.map((item) => {
            ...
            const anyChildActive = item.children?.some(c => pathname.startsWith(c.href) && c.href !== '/dashboard')
            ...
            return (
              <div key={item.label}>
                ...
                onMouseLeave={e => { if (!anyChildActive) e.currentTarget.style.background = anyChildActive ? '#EEF2FF' : 'transparent' }}>
                  <i className={'ti ' + item.icon} style={{ fontSize:17, color: anyChildActive ? '#1B2B5B' : '#A1A1AA', flexShrink:0 }}/>
                  <span style={{ fontSize:13, fontWeight: anyChildActive ? 500 : 400, color: anyChildActive ? '#1B2B5B' : '#52525B', flex:1 }}>{item.label}</span>
                ...
                {abierto && (
                  <div style={{ paddingLeft:14, paddingBottom:4 }}>
                    {item.children?.map(child => {
                      ...
                      return (
                        <Link key={child.label + child.href} href={child.href} style={{
                          display:'flex', alignItems:'center', gap:10,
                          padding:'7px 10px 7px 14px', margin:'1px 8px 1px 0',
                          borderRadius:7, textDecoration:'none',
                          background: active ? '#EEF2FF' : 'transparent',
                          borderLeft: `2px solid ${active ? '#1B2B5B' : 'transparent'}`,
                          transition:'all 0.15s',
                        }}
