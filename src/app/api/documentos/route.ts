...
export async function POST(req: Request) {
  try {
    ...
    if (!tipo || !nombre) return NextResponse.json({ error: 'Faltan datos.' }, { status: 400 })

    const outDir = path.join(process.cwd(), 'public', 'documentos')
    if (!existsSync(outDir)) mkdirSync(outDir, { recursive: true })

    const archivo = `SYNAP_${tipo}_${String(serial).replace(/[^a-zA-Z0-9]/g, '_')}.pdf`
    const outPath = path.join(outDir, archivo).replace(/\\/g, '/')
    const logoPath = path.join(process.cwd(), 'public', 'synap_logo.jpg').replace(/\\/g, '/')
    const scriptPath = path.join(process.cwd(), 'tmp_doc.py')

    const n = nombre.replace(/'/g, '')
    const ma = (marca || 'N/D').replace(/'/g, '')
    const mo = (modelo || 'N/D').replace(/'/g, '')
    const re = (referencia || 'N/D').replace(/'/g, '')
    const se = (serial || 'N/D').replace(/'/g, '')
    const sv = (servicio || 'N/D').replace(/'/g, '')
...
NOMBRE='${n}'; MARCA='${ma}'; MODELO='${mo}'; REF='${re}'; SERIAL='${se}'; SVCIO='${sv}'
...
def lc():
  try: return Image('${logoPath}',width=3.5*cm,height=1.2*cm)
