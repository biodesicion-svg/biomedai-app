...
export default function ProtocolosPage(){
  ...
  const[selProt,setSelProt]=useState<any>({id:'',nombre:'Cargando...',clase:'-',riesgo:'medio',frecuencia:'-',duracion:'-',normativa:'IEC 60601, Res. 4816/2008',categorias:[],pasos:[]})
  const[catSel,setCatSel]=useState('todas')
  const[busqueda,setBusqueda]=useState('')

  useEffect(()=>{
    fetch('/api/protocolo').then(r=>r.json()).then(d=>{
      const ps=(d.protocolos||[]).map((p:any)=>({
        id:p.clave, nombre:p.nombre, clase:p.clase_invima||'-',
        riesgo:(p.clase_invima==='IIb'||p.clase_invima==='III')?'alto':(p.clase_invima==='IIa'?'medio':'bajo'),
        frecuencia:p.frecuencia||'-', duracion:(p.duracion_horas||0)+'h',
        normativa:'IEC 60601, Res. 4816/2008', categorias:[], pasos:[], total_pasos:p.total_pasos||0, criticos:p.criticos||0,
      }))
      setLista(ps)
      if(ps.length) cargarPasos(ps[0])
    }).catch(()=>{})
  },[])

  async function cargarPasos(p:any){
    try{
      const r=await fetch('/api/protocolo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({equipo:p.nombre,tipo:'preventivo'})})
      const d=await r.json()
      const pasos=(d.preguntas||[]).map((q:any)=>({
        n:q.numero, cat:q.categoria, pregunta:q.pregunta, tipo:q.tipo,
        opciones:q.opciones, esperado:q.valor_esperado, unidad:q.unidad,
        critica:q.critica, adv:q.advertencia||null,
      }))
      const cats=Array.from(new Set(pasos.map((x:any)=>x.cat)))
      setSelProt({...p, pasos, categorias:cats})
      setCatSel('todas'); setBusqueda('')
    }catch(e){ setSelProt({...p, pasos:[], categorias:[]}) }
  }

  const pasosFiltrados=(selProt.pasos||[]).filter((p:any)=>{
    if(catSel!=='todas'&&p.cat!==catSel) return false
    if(busqueda&&!p.pregunta.toLowerCase().includes(busqueda.toLowerCase())) return false
    return true
  })

  const criticos=(selProt.pasos||[]).filter((p:any)=>p.critica).length
    ...
    <div style={{display:'flex',height:'100vh',overflow:'hidden',background:'#fff'}}>
      ...
      <div style={{width:260,flexShrink:0,background:'#fff',borderRight:'0.5px solid #E4E4E7',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'16px 14px',borderBottom:'0.5px solid #E4E4E7'}}>
          ...
          <div style={{fontSize:11,color:'#A1A1AA',marginTop:2}}>{lista.length} protocolos disponibles</div>
        </div>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'10px'}}>
          ...
          {lista.map((p:any)=>{
            ...
            return(
              <button key={p.id} onClick={()=>cargarPasos(p)}
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:'flex',gap:5,marginTop:2}}>
                    <span style={{fontSize:10,color:rCol}}>{p.total_pasos ?? p.pasos.length} pasos</span>
